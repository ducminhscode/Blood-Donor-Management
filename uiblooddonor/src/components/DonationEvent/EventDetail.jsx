                ...
                <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full transform transition-all">
                    ...
                    <div className="p-6">
                        <div className="flex flex-col items-center text-center mb-6">
                            <div className="w-24 h-24 bg-gradient-to-br from-red-100 to-red-200 rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-lg">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-gray-50 rounded-xl p-3">
                                    ...
                                    <span
                                        className="text-sm font-medium text-gray-900 flex items-center gap-1 truncate"
                                    >
...
const RegistrationDialog = ({ isOpen, onClose, onSubmit, user, event, donorInfo }) => {
    ...
    // Fetch province name for display
    useEffect(() => {
    ...
    // Fetch district name for display
    useEffect(() => {
    ...
    // CHỈ cập nhật formData từ donorInfo khi KHÔNG phải là proxy
    useEffect(() => {
        // CHỈ chạy khi không phải proxy và có donorInfo
    ...
    }, [donorInfo, formData.is_proxy, isOpen]); // Thêm formData.is_proxy vào dependency array

    // Khi chuyển đổi is_proxy, reset các trường thông tin
    useEffect(() => {
        if (formData.is_proxy) {
            // Khi chuyển sang proxy, reset tất cả các trường thông tin
            setFormData(prev => ({
                ...prev,
                permanent_address: '',
                career: '',
            ...
            // Khi chuyển về không proxy, lấy lại thông tin từ donorInfo
            setFormData(prev => ({
                ...prev,
                permanent_address: donorInfo.permanent_address || '',
                career: donorInfo.career || '',
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-4">
                        ...
                        {!formData.is_proxy ? (
                            <>
                                ...
                                {/* Thông tin từ donor profile - DISPLAY ONLY */}
                                <div className="bg-gray-50 p-4 rounded-xl">
                                    <h4 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
...
const EventDetail = () => {
    ...
    // State cho danh sách sự kiện cùng khu vực
    const [relatedEvents, setRelatedEvents] = useState([]);
    const [loadingRelated, setLoadingRelated] = useState(false);

    // State cho tên tỉnh/thành
    const [provinceName, setProvinceName] = useState('');
    const [districtName, setDistrictName] = useState('');
    ...
    // Fetch các sự kiện cùng khu vực khi có event
    useEffect(() => {
        if (event?.province) {
    ...
    // Hàm fetch các sự kiện cùng khu vực (loại trừ sự kiện hiện tại)
    const fetchRelatedEvents = async (province) => {
        ...
        try {
            ...
            // Lọc bỏ sự kiện đang xem và giới hạn 3
            const filteredEvents = events
                .filter(e => e.id !== parseInt(id)) // Loại trừ sự kiện hiện tại
                .slice(0, 3);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <div className="relative overflow-hidden bg-gradient-to-r from-red-600 via-red-500 to-orange-500 text-white">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    <nav className="flex items-center gap-2 text-sm text-white/80 mb-6">
                        <span>Sự kiện</span>
                        <span>/</span>
                        <span className="text-white">{event.title || 'Chi tiết'}</span>
                    </nav>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        >
                            ...
                            <span className='mr-1'>Sự kiện</span>
                        </button>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6">
                            <div className="flex items-start justify-between mb-4">
                                <div>
                                    <div className="flex items-center gap-3 mb-3">
                                        <span className={`${getStatusColor(event)} text-white px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1`}>
                                            {getStatusText(event)}
                                        </span>
                                    </div>
                            ...
                            <div className="lg:hidden mt-4">
                                {!user ? (
                                    <button
                                        onClick={() => navigate('/login')}
                                        className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all shadow-lg bg-gray-600 text-white hover:bg-gray-700 shadow-gray-500/25"
                                    >
                                        <span>Đăng nhập để tham gia</span>
                                    </button>
                                ) : user.role === 1 ? (
                                ...
                                {localStorage.getItem('access_token') && (
                                    <div className="mt-3 flex justify-between text-sm bg-gray-50 rounded-lg p-2">
                                        <div className="flex items-center gap-1">
                                            <Users className="w-4 h-4 text-gray-500" />
                                            <span className="text-gray-600">Đã ĐK: {event.registered_count || 0}</span>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Target className="w-4 h-4 text-gray-500" />
                                            <span className="text-gray-600">Chỉ tiêu: {event.target || 200}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="border-b border-gray-200 px-6">
                                <div className="flex gap-6">
                                    ...
                                    <button
                                        ...
                                        className={`py-4 px-2 font-medium transition-all relative ${activeTab === 'location'
                                            ? 'text-red-600'
                                            : 'text-gray-500 hover:text-gray-700'
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`py-4 px-2 font-medium transition-all relative ${activeTab === 'organizer'
                                            ? 'text-red-600'
                                            : 'text-gray-500 hover:text-gray-700'
                                            }`}
                                    >
                                ...
                                {activeTab === 'organizer' && (
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                                            <div className="w-16 h-16 bg-gradient-to-br from-red-100 to-red-200 rounded-full flex items-center justify-center overflow-hidden">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <InfoRow
                                                icon={Phone}
                                                label="Hotline"
                                                value={`${event.staff?.emergency_phone || ""}`}
                                            />
                                        </div>
                                    </div>
                                )}
                ...
                {/* Related Events - Sự kiện cùng khu vực */}
                        ...
                        {loadingRelated ? (
                            ...
                            <div className="grid md:grid-cols-3 gap-6">
                                {relatedEvents.map((relatedEvent) => (
                                    <div
                                        key={relatedEvent.id}
                                        className="bg-white rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer transform hover:-translate-y-1"
                                        onClick={() => navigate(`/event/${relatedEvent.id}`)}
                                    >
                                        <div className="h-32 bg-gradient-to-br from-red-400 to-red-600 rounded-t-xl relative overflow-hidden">
                                            ...
                                            <div className="absolute top-2 right-2">
                                                <span className={`${getStatusColor(relatedEvent)} text-white text-xs px-2 py-1 rounded-full`}>
                                        ...
                                        <div className="p-4">
                                            ...
                                            <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                                                ...
                                                {relatedEvent.location}, {relatedEvent.sub_district}, {relatedEvent.province}
                                            </p>
                                        </div>
                                    </div>
                                ))}
                        ...
                        {relatedEvents.length > 0 && (
                            <div className="text-center mt-6">
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-6 py-3 bg-white text-red-600 rounded-xl font-semibold hover:bg-red-50 transition-all shadow-sm"
                                >
                                    Xem thêm sự kiện
