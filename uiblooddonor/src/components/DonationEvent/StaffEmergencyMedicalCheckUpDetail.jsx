...
const EditMedicalCheckupDialog = ({ isOpen, onClose, onSubmit, checkup, loading }) => {
    ...
    const handleResultSelect = (isEligible) => {
        setFormData(prev => ({
            ...prev,
    ...
    const getResultLabel = (isEligible) => {
                                ...
                                {showResultDropdown && (
                                    <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-30 animate-fadeIn">
                                        <button
                                            ...
                                            className={`cursor-pointer w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors ${formData.is_eligible === true ? 'bg-red-50 text-red-600' : 'text-gray-700'}`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`cursor-pointer w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors ${formData.is_eligible === false ? 'bg-red-50 text-red-600' : 'text-gray-700'}`}
                                        >
...
const StaffEmergencyMedicalCheckUpDetail = () => {
    ...
    const patientName = responseDetail?.emergency_request?.patient_name || 'Đang tải tên bệnh nhân';
    const donorName = `${responseDetail?.donor?.account?.last_name || ''} ${responseDetail?.donor?.account?.first_name || ''}`.trim() || 'Đang tải người hiến máu';
                                ...
                                {canEditOrDelete() && !isCompleted && (
                                    <div className="flex flex-wrap gap-3 justify-end">
                                        <button
                                            ...
                                            className="cursor-pointer cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all duration-300 font-medium"
                                        >
